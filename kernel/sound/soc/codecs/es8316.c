...
static const struct reg_default es8316_reg_defaults[] = {
	...
	{0x20, 0x00}, {0x21, 0x00}, {0x22, 0xd0}, {0x23, 0x00},
			      ...
			      unsigned int fmt)
{
	...
	// iface    = snd_soc_component_read32(component, ES8316_IFACE);
	// adciface = snd_soc_component_read32(component, ES8316_ADC_IFACE);
	// daciface = snd_soc_component_read32(component, ES8316_DAC_IFACE);
...
static int es8316_mute(struct snd_soc_component *component, int mute, int direction)
{
	struct es8316_priv *es8316 = snd_soc_component_get_drvdata(component);
...
static int es8316_probe(struct snd_soc_component *component)
{
	...
	if (!ret) {
		...
		if (!ret) {
			...
			snd_soc_component_write(component, ES8316_CLKMGR_CLKSW_REG01, 0xF3);
			snd_soc_component_write(component,
				      ES8316_ADC_PDN_LINSEL_REG22, 0xd0);
		}
	}
